...
public enum BitmapManager {
	...
	public void queueJob(final String url, final ImageView imageView,
			final int width, final int height, final Bitmap defaultThumb, final boolean rounded, final int roundedPixel) {
		...
		final Handler handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (tag != null && tag.equals(url)) {
					if (msg.obj != null) {
						setBitmapAnimated(imageView, (Bitmap) msg.obj, rounded, roundedPixel);
					} else {
		...
		pool.submit(new Runnable() {
			public void run() {
				final Bitmap bmp = downloadBitmap(url, width, height, rounded, roundedPixel);
	...
	private void setBitmapAnimated(final ImageView view, final Bitmap bitmap, final boolean rounded, final int roundedPixel){
	...
	public void loadBitmap(final String url, final ImageView imageView,
			final int width, final int height, Bitmap defaultThumb, boolean rounded, int roundedPixel) {
		...
		if (bitmap != null) {
			...
			queueJob(url, imageView, width, height, defaultThumb, rounded, roundedPixel);
		}
	}
	...
	private Bitmap downloadBitmap(String url, int width, int height, boolean rounded, int roundedPixel) {
		Bitmap bitmap = FetchBitmapFromURL(url);
		if (width > 0 && height > 0) {
			bitmap = Bitmap.createScaledBitmap(bitmap, width, height, true);
			if(rounded) bitmap = ImageUtil.getRoundedCornerBitmap(bitmap, roundedPixel);
		}
